...
def main():
    ...
    with ProcessPoolExecutor(max_workers=6) as pool:
        ...
        for number, future in enumerate(as_completed(futures), 1):
            ...
            if number % 20 == 0:
                CACHE.write_text(json.dumps({"version": "3.6.1", "pages": completed}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
                print(f"OCR {number}/{len(jobs)}", flush=True)
    CACHE.write_text(json.dumps({"version": "3.6.1", "pages": dict(sorted(completed.items(), key=lambda x: int(x[0])))}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
